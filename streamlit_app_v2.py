...
CHAIN_HINTS = {
    ...
    "target", "shell", "burger king", "taco bell", "wendy's"
}

NO_MAPS_TERMS = {"weather", "time", "facebook", "instagram", "youtube"}
...
def classify_query(query: str) -> QueryIntent:
    ...
    if not q:
        return QueryIntent("NO_MAPS_INTENT", None, False, False, False, q)

    if any(term == ql for term in NO_MAPS_TERMS):
        return QueryIntent("NO_MAPS_INTENT", None, False, False, False, q)

    if re.fullmatch(r"\s*-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?\s*", q):
        return QueryIntent("COORDINATE", None, False, False, True, q)

    has_near_me = any(p in ql for p in ("near me", "nearest", "nearby", "closest"))
    has_address_number = bool(re.search(r"\b\d+\s+\w+", ql))
    ...
    is_chain = any(name in ql for name in CHAIN_HINTS)
    transit = any(t in ql for t in ("station", "airport", "terminal", "metro", "subway"))
    locality = bool(re.fullmatch(r"[A-Za-z .'-]+", q)) and ql in {
        "soho", "cork", "toronto", "seattle", "london", "upper east side"
    }

    if has_address_number:
        return QueryIntent("ADDRESS", explicit_location, has_near_me, is_chain, True, q)
    if transit:
        return QueryIntent("TRANSIT", explicit_location, has_near_me, is_chain, True, q)
    if locality:
        return QueryIntent("LOCALITY", explicit_location, has_near_me, is_chain, True, q)
    if is_chain:
        probably_unique = explicit_location is not None and not has_near_me
        return QueryIntent("POI_OR_BUSINESS", explicit_location, has_near_me, True, probably_unique, q)

    category_terms = ("zoo", "hotel", "bank", "hospital", "gas station", "pharmacy", "mall", "park")
    if any(t in ql for t in category_terms):
        return QueryIntent("CATEGORY", explicit_location, has_near_me, False, False, q)

    return QueryIntent("POI_OR_BUSINESS", explicit_location, has_near_me, False, False, q)
